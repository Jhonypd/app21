...
interface MultiSelectCommandProps {
   ...
   error?: boolean;
   disabled?: boolean;
   onSearch?: (term: string, excludeIds?: string[]) => Promise<Option[]>;
}
...
export const MultiSelectCommand: React.FC<MultiSelectCommandProps> = ({
   ...
   const filteredOptions = useMemo(() => {
      return remoteOptions.filter((ro) => !value.some((v) => v.id === ro.id));
   }, [remoteOptions, value]);

   // Debounce para a busca
   const debouncedSearch = useCallback(
      debounce(async (term: string, excludeIds: string[] = []) => {
         // ↓↓↓ VALIDAÇÃO DE MÍNIMO 3 CARACTERES ↓↓↓
         if (onSearch && term.trim().length >= 3) {
            setLoading(true);
            try {
               const results = await onSearch(term, excludeIds);
      ...
      >
         ...
         {value.length > 0 && (
            <div className="mt-3 gap-2">
               <p className="text-sm text-gray-400">Usuários selecionados:</p>
