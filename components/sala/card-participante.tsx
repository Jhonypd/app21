...
interface CardParticipanteProps {
   ...
   onAlterarRole?: (pessoaId: string, novoRole: 1 | 2) => void;
   onRemover?: (participante: { id: string; nome: string }) => void;
   voto?: string | null;
   votosRevelados?: boolean;
   layout?: 'horizontal' | 'vertical';
   votoId?: string;
   onAnularVoto?: (votoId: string, nomeParticipante: string) => void;
}
...
export function CardParticipante({
   ...
   const fundoAvatar =
      participante && participante.role >= 2
         ...
         : getAvatarGradientPorRole(participante.id, participante.role);
            ...
            <CardContent className="flex flex-col items-center gap-3 p-4 text-center">
               ...
               <div className="w-full space-y-1">
                  <Tooltip>
                     ...
                     <TooltipContent>{participante.nome}</TooltipContent>
                  </Tooltip>
               ...
               <div className="flex w-full items-center justify-center gap-2">
                  ...
                  >
                     <p className="flex h-8 w-8 items-center justify-center rounded-full bg-green-500 text-lg font-medium">
                        {voto ? <span>{voto}</span> : <TimerIcon size={18} />}
                     </p>

                     {onAnularVoto && (meuRole === 0 || meuRole === 1) && (
                        <Button
                           variant={'destructive'}
                           size={'icon'}
                           onClick={() =>
                           ...
                           title="Anular voto"
                           disabled={!voto || !votoId || votosRevelados}
                        >
               ...
               {/* Ações */}
               {mostrarAcoes && (podeAlterarRole || podeRemoverEste) && (
         ...
         <CardContent className="flex items-center gap-4 p-4">
                  ...
                  {voto && votosRevelados ? (
                    ...
                    (meuRole === 0 || meuRole === 1) ? (
                     ...
                     <div className="bg-primary/20 flex items-center gap-1.5 rounded-lg px-2 py-1">
                        ...
                        <span className="text-primary text-xs">Votou</span>
                     </div>
                  ) : (
            ...
            {mostrarAcoes && (
               <div className="flex flex-shrink-0 flex-col gap-2">
                  {/* Alterar Role */}
                  {podeAlterarRole && !isDono && jaExistia && onAlterarRole && (
                     <div className="flex gap-1">
                        {participante.role === 2 && (
                           <button
                              onClick={() => onAlterarRole(participante.id, 1)}
                        ...
                        {participante.role === 1 && (
                           <button
                              onClick={() => onAlterarRole(participante.id, 2)}
