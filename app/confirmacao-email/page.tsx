...
import { toastError, toastInfo, toastSuccess } from '@/components/custom-toast';
...
const ConfirmacaoEmail = () => {
   ...
   const [novoCodigo, { isLoading }] = useNovoCodigoMutation();
   ...
   useEffect(() => {
      const confirmar = async () => {
         ...
         try {
            ...
            if (result?.Sucesso) {
               const contaConfirmada = result?.Resultado?.contaConfirmada;
               ...
               if (contaConfirmada === true) {
                  ...
                  toastInfo({
                     description:
                        result?.Mensagem ?? 'Código validado com sucesso.',
                  });
               }
               return;
            }
            ...
            toastError({
               description: result?.Mensagem ?? 'Falha ao confirmar email.',
            });
         } catch (err: unknown) {
            toastError({
               description:
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         ...
         toastInfo({
            description: result?.Mensagem ?? 'Código reenviado.',
         });
      } catch (err: unknown) {
         toastError({
            description:
               getApiErrorMessage(err) ?? 'Erro ao tentar reenviar o código.',
         });
      }
   };
      ...
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-4">
         <div className="w-full max-w-md">
            ...
            <Card className="shadow-lg">
               <CardHeader>
                  <CardTitle className="text-xl">Reenviar código</CardTitle>
                  <CardDescription>
                     Não recebeu o email? Digite seu endereço abaixo para
                     reenviar.
                  </CardDescription>
               </CardHeader>
               <CardContent>
